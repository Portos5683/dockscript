...
direpaiement=/home/$userepaiement
...
##************************************************* Si Vault est deja installer
   ...
   if [ -e "/etc/vault/init.file" ]
   then
###************************************************ Si le fichier de sauvegarde est trouver
      echo "Extraction des clés et du jeton"
      tokenline=`sed '7!d' /etc/vault/init.file`
      token=${tokenline:20:50}
      vaultkey1=`sed '1!d' /etc/vault/init.file`
...
###************************************************** Si le fichier de sauvegarde des cles n'est pas trouver
      token=""
      while [ "$token" = "" ]
      do
         read -p "Saisissez la valeur du token : " $token
...
## Si le token a une valeur vide
   ...
   if [ "$key1" = "" ]
   then
###***************************************************** Si la valeur de la premier cle est nul
      if [ "$key2" = "" ]
      then
####**************************************************** Si la valeur de la deuxieme cle est nul 
         if [ "$key3" = "" ]
         then
#####*************************************************** Si la valeur de la troisieme cle est nul
            echo "------------------------------------------"
            echo ""
            echo "ERROR DE SAISIE:  Vous n'avez entrer aucune cle de deverrouillage "
            echo " (key1 : $key1),"
...
function otrInsertData()
{
   ...
   do
      ...
      echo "----------------------------------------------------------------------------"
      echo ""
      echo "Insertion des informations pour la configuration de vault --> otr"
      echo ""
      echo "----------------------------------------------------------------------------"
      echo ""
      echo "Veuillez insérer l'adresse email de l'administrateur (eg: admin@mail.com)"
   ...
   ##########perl -i -p -e 's|"admin.email": .*|"admin.email": "'$adminemail'",|g' "test.json"
   echo "Insertion des donnees dans le fichiers otr.json"
   path=`pwd`
   sed -i 's/\("admin.email":\).*/\"admin.email": "'$adminemail'",/' $path/otr.json
   sed -i 's/\("connector.amplitude.host": \).*/\"connector.amplitude.host": "'$connectoramplitudehost'",/' $path/otr.json
   sed -i 's/\("gateway.url": \).*/\"gateway.url": "'$gatewayurl'",/' $path/otr.json
   sed -i 's/\("spring.datasource.hostname": \).*/\"spring.datasource.hostname": "'$springdatasourcehostname'",/' $path/otr.json
   sed -i 's/\("spring.datasource.password": \).*/\"spring.datasource.password": "'$springdatasourcepassword'",/' $path/otr.json
   sed -i 's/\("spring.datasource.port": \).*/\"spring.datasource.port": "'$springdatasourceport'",/' $path/otr.json
   sed -i 's/\("spring.datasource.username": \).*/\"spring.datasource.username": "'$springdatasourceusername'",/' $path/otr.json
   sed -i 's/\("spring.mail.from": \).*/\"spring.mail.from": "'$springmailfrom'",/' $path/otr.json
   sed -i 's/\("spring.mail.host": \).*/\"spring.mail.host": "'$springmailhost'",/' $path/otr.json
   sed -i 's/\("spring.mail.password": \).*/\"spring.mail.password": "'$springmailpassword'",/' $path/otr.json
   sed -i 's/\("spring.mail.port": \).*/\"spring.mail.port": "'$springmailport'",/' $path/otr.json
   sed -i 's/\("spring.mail.username": \).*/\"spring.mail.username": "'$springmailusername'",/' $path/otr.json
   sed -i 's/\("spring.redis.host": \).*/\"spring.redis.host": "'$springredishost'",/' $path/otr.json
   sed -i 's/\("spring.redis.password": \).*/\"spring.redis.password": "'$springredispassword'",/' $path/otr.json
   sed -i 's/\("spring.redis.port": \).*/\"spring.redis.port": "'$springredisport'",/' $path/otr.json
   vault kv put $engine/epaiement-online @$path/otr.json
   sure=1
   response=n
}
...
###***************************************************** Si l'option 1 est choisi "Insertion de donnees"
      ...
      nbreargument=$#;
      firstarg=$1
      if ([ $nbreargument = 0 ] || [[ $firstarg = exit && $nbreargument = 1]]); then
